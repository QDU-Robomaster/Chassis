...
class Mecanum {
 ...
 Mecanum(LibXR::HardwareContainer &hw, LibXR::ApplicationManager &app,
  ...
  static void ThreadFunction(Mecanum *mecanum) {
    ...
    LibXR::Topic::ASyncSubscriber<PowerControl<Mecanum>::PowerControlData>powercontrol_data_suber("powercontrol_data");
    ...
    while (true) {
      ...
      mecanum->topic_motor_data_.Publish(mecanum->motor_data_);
      mecanum->mutex_.Unlock();
  ...
  void InverseKinematicsSolution() {
    target_motor_omega_[0] = (- target_vx_ + target_vy_ +
