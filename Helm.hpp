...
class Helm {
 public:
  Helm(LibXR::HardwareContainer &hw, LibXR::ApplicationManager &app, CMD &cmd,
       Motor<MotorType> &motor_can1, Motor<MotorType> &motor_can2, uint32_t task_stack_depth,
       LibXR::PID<float>::Param pid_param)
      : motor_can1_(&motor_can1),
        motor_can2_(&motor_can2),
        cmd_(&cmd),
        pid_param_(pid_param) {
    thread_.Create(this, ThreadFunction, "HelmThread", task_stack_depth,
  ...
  static void ThreadFunction(Helm *helm) {
    ...
    while (true) {
      ...
      helm->SteerMotorKinematicsNearestTransposition();
