...
#ifndef OMNI_CHASSIS_DEBUG_IMPL
#include "Omni.hpp"
#endif

inline int Omni::DebugCommand(int argc, char** argv) {
  ...
  static const debug_core::LiveFieldDesc<Omni> fields[] = {
      DEBUG_CORE_LIVE_CUSTOM(
          Omni, "mode", mask_state,
          +[](const char* field_name, const Omni* self) {
      ...
      DEBUG_CORE_LIVE_BOOL(Omni, "cmd_online", mask_state,
                           self->cmd_ != nullptr && self->cmd_->Online()),
      DEBUG_CORE_LIVE_F32(Omni, "dt_s", mask_state, self->dt_),
      DEBUG_CORE_LIVE_F32(Omni, "chassis_yaw", mask_state, self->chassis_yaw_),
      DEBUG_CORE_LIVE_F32(Omni, "imu_yaw", mask_state, self->current_yaw_),
      DEBUG_CORE_LIVE_F32(Omni, "imu_pitch", mask_state, self->current_pitch_),
      DEBUG_CORE_LIVE_F32(Omni, "imu_roll", mask_state, self->current_roll_),
      DEBUG_CORE_LIVE_F32(Omni, "cmd_x", mask_cmd, self->cmd_data_.x),
      DEBUG_CORE_LIVE_F32(Omni, "cmd_y", mask_cmd, self->cmd_data_.y),
      DEBUG_CORE_LIVE_F32(Omni, "cmd_z", mask_cmd, self->cmd_data_.z),
      DEBUG_CORE_LIVE_F32(Omni, "target_vx", mask_motion, self->target_vx_),
      DEBUG_CORE_LIVE_F32(Omni, "target_vy", mask_motion, self->target_vy_),
      DEBUG_CORE_LIVE_F32(Omni, "target_omega", mask_motion,
                          self->target_omega_),
      DEBUG_CORE_LIVE_F32(Omni, "now_vx", mask_motion, self->now_vx_),
      DEBUG_CORE_LIVE_F32(Omni, "now_vy", mask_motion, self->now_vy_),
      DEBUG_CORE_LIVE_F32(Omni, "now_omega", mask_motion, self->now_omega_),
      DEBUG_CORE_LIVE_CUSTOM(
          Omni, "wheel_motion", mask_motion,
          +[](const char* field_name, const Omni* self) {
      ...
      DEBUG_CORE_LIVE_BOOL(Omni, "power_limited", mask_power,
                           self->power_control_data_.is_power_limited),
      DEBUG_CORE_LIVE_CUSTOM(
          Omni, "power_current", mask_power,
          +[](const char* field_name, const Omni* self) {
            LibXR::STDIO::Printf("  %s:\r\n", field_name);
            for (int i = 0; i < 4; ++i) {
      ...
      DEBUG_CORE_LIVE_CUSTOM(
          Omni, "wheel_feedback", mask_wheel,
          +[](const char* field_name, const Omni* self) {
            LibXR::STDIO::Printf("  %s:\r\n", field_name);
            for (int i = 0; i < 4; ++i) {
  ...
  auto lock_self = +[](Omni* self) { self->mutex_.Lock(); };
  auto unlock_self = +[](Omni* self) { self->mutex_.Unlock(); };

  return debug_core::run_live_command(
      this, "omni_chassis", "state|cmd|motion|power|wheel|full", view_table,
